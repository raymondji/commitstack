...
- `git stack` is designed to function as a minimal addition to the Git CLI. It works with existing Git concepts and functionalty (like `--update-refs`), and aims to unintrusively fill in the gaps. Unlike most stacking tools, it's also stateless, so there's no need to keep state in sync between Git and `git stack`. Instead, it works by automatically inferring stacks from the structure of your commits.
...
```
go install github.com/raymondji/git-stack-cli/cmd/git-stack@0.16.0
```
