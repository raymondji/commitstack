...
On the flipside, one reason Commitstack might not be a good fit is if you prefer to avoid `git rebase`. Commitstack uses `git rebase --update-refs` internally, and users will need to be comfortable doing so directly as well.
